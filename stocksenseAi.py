...
class StockAnalyzer:
    def __init__(self):
        ...
        self.mid_cap_stocks = [
            ...
            'VOLTAS.NS', 'PAGEIND.NS', 'MPHASIS.NS', 'LTIM.NS', 'LTTS.NS', # MINDTREE might be LTIM now
            'PERSISTENT.NS', 'COFORGE.NS', 'BIOCON.NS', 'LUPIN.NS', 'TORNTPHARM.NS',
            'AUBANK.NS', 'FEDERALBNK.NS', 'BANDHANBNK.NS', 'IDFCFIRSTB.NS', 'MOTHERSON.NS', # MOTHERSUMI is now MOTHERSON
            'ASHOKLEY.NS', 'ESCORTS.NS', 'EXIDEIND.NS', 'AMARAJABAT.NS', 'TVSMOTOR.NS', # AMARA.NS -> AMARAJABAT.NS
            'BALKRISIND.NS', 'APOLLOTYRE.NS', 'MRF.NS', 'CUMMINSIND.NS', 'BATAINDIA.NS',
            'RELAXO.NS', 'VBL.NS', 'TATACONSUM.NS', 'JUBLFOOD.NS', 'CROMPTON.NS',
            'WHIRLPOOL.NS', 'SIEMENS.NS', 'GODREJPROP.NS', 'OBEROIRLTY.NS', 'DLF.NS', # Replaced one VOLTAS.NS with SIEMENS.NS
            'PRESTIGE.NS', 'BRIGADE.NS', 'SOBHA.NS', 'PHOENIXLTD.NS', 'PVRINOX.NS' # INOXLEISUR.NS -> PVRINOX.NS (post-merger)
        ]
        
        self.small_cap_stocks = [
            'AFFLE.NS', 'ROUTE.NS', 'INDIAMART.NS', 'ZOMATO.NS', 'PAYTM.NS', # NAUKRI.NS -> INFOEDGE.NS (parent), used INDIAMART.NS as alt
            'POLICYBZR.NS', 'FSL.NS', 'CARBORUNIV.NS', 'PGHL.NS', 'VINATIORGA.NS',
            'SYMPHONY.NS', 'RAJESHEXPO.NS', 'ASTRAL.NS', 'NILKAMAL.NS', 'CERA.NS',
            'JKCEMENT.NS', 'RAMCOCEM.NS', 'HEIDELBERG.NS', 'PRISMCEM.NS', 'SUPRAJIT.NS', # PRISM.NS -> PRISMCEM.NS
            'SCHAEFFLER.NS', 'TIMKEN.NS', 'SKFINDIA.NS', 'NRBBEARING.NS', 'FINEORG.NS', # NRB.NS -> NRBBEARING.NS
            'AAVAS.NS', 'HOMEFIRST.NS', 'UJJIVANSFB.NS', 'SPANDANA.NS', 'CREDITACC.NS', # UJJIVAN.NS -> UJJIVANSFB.NS
            'LAXMIMACH.NS', 'TEXRAIL.NS', 'KNRCON.NS', 'IRB.NS', 'SADBHAV.NS',
            'GPPL.NS', 'ZFSTEERING.NS', 'REDINGTON.NS', 'DELTACORP.NS', 'ORIENTCEM.NS', # WABCOINDIA.NS -> ZFSTEERING.NS (or similar auto ancillary)
            'CENTURYPLY.NS', 'GREENPLY.NS', 'KANSAINER.NS', 'AIAENG.NS', 'THERMAX.NS',
            'KIRLOSENG.NS', 'GRINDWELL.NS', 'CARYSIL.NS', 'HINDWAREAP.NS', 'DIXON.NS' # HINDWARE.NS -> HINDWAREAP.NS
        ]
